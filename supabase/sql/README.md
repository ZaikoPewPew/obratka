...
| `review_complaints.sql` | reputation (старт 0, бан −100, 1 тег, окно 6ч от done, +10 settle) + RPC complaint. После apply: закомментировать ONE-SHOT `reputation 20→0` |
...
| Окно жалобы от done + старт reputation 0 | [`portfolios.sql`](portfolios.sql) (колонка `completed_at`) → [`review_claims.sql`](review_claims.sql) (триггер) → весь [`review_complaints.sql`](review_complaints.sql); после первого apply **закомментировать** ONE-SHOT `reputation 20→0` |
