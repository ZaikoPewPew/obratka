...
| Сегмент «Уже отревьюено» / done-карточки после своего ревью | блок `has_reviewed_portfolio` + `portfolios_select_feed` из [`portfolios.sql`](portfolios.sql) (`security definer`; **не** сырой `exists` по `reviews` — circular RLS) |
