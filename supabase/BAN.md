...
После бана пользователь попадёт на `/banned` при следующем обновлении вкладки, навигации или возврате в браузер (не мгновенный push). History / deep link на другие экраны схлопываются на `/banned`. Сессию Auth бан сам не отзывает — «Выйти» на экране или удаление аккаунта.

Удаление через `DELETE auth.users` (см. `delete-account-templates.sql`): клиент при мёртвой Auth вылогинивает на `/referral` (способ входа), gated-экраны по history недоступны.
