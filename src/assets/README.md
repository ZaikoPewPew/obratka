...
`plus.svg` / `report-sent.svg` / `scroll-top.svg` — inline через `?raw` (`currentColor`).
...
FAB feedback — глаза в [`feedback`](../components/feedback/README.md).
...
| `welcome-reels.MOV` | шаг video онбординга (4 из 4); ключ `welcome` |
