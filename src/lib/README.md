...
| [`analytics.js`](analytics.js) | PostHog facade (`initAnalytics` / `trackPage` / `track` / `identifyUser` / `resetAnalytics`). Pageviews из `applyRoute`; без `VITE_POSTHOG_KEY` — no-op. SoT: [`ANALYTICS.md`](../../ANALYTICS.md). |
