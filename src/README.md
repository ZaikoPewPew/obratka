...
- `main.js` — точка входа: OAuth/boot-проверка Auth, роутер (`go` / `applyRoute`), home query, монтаж экранов, **desktop-only gate** (`viewport.js` + `desktop-only-screen`), iframe-таймер (iframe pause / external wall-clock + `Timer-end.wav`), **надиктовка** (чип rec + микрофон в поле совета) + post-edit через `dictationPolish`, квиз; email/Telegram завершаются через `AuthScreen` → `applyProviderUser`.
