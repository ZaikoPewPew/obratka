...
| `dictationPolish.js` | Edge `polish-dictation`: post-edit Web Speech текста (пунктуация / пробелы / регистр; **не** STT; default `glm-4.5-flash`). `polishDictationText(text, { maxLen, locale })` → при ошибке/таймауте/всех упавших моделях **сырой** текст (submit не блокируется). Сейчас **выключен** (`POLISH_ENABLED = false` — без invoke; вернуть `true` чтобы снова). Секрет `ZAI_API_KEY` только в Function. SoT: [`supabase/functions/polish-dictation/README.md`](../../supabase/functions/polish-dictation/README.md) |
