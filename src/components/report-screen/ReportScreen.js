...
export function createReportScreen(opts = {}) {
  ...
  /**
   * @returns {{ answers: import("../../utils/reviewReport.js").ReviewAnswers; reviewerName: string }[]}
   */
  function consensusInputs() {
    const t = getStrings();
    /** @type {{ answers: import("../../utils/reviewReport.js").ReviewAnswers; reviewerName: string }[]} */
    const rows = [];
    for (const sheet of sheets) {
      if (!sheet.answers) continue;
      const reviewerName =
   ...
   * @returns {number}
   */
  function currentSheetLimit() {
    const fromSheet = sheets.find((sheet) => Number.isFinite(sheet.targetReviews));
    if (fromSheet && Number.isFinite(fromSheet.targetReviews)) {
  ...
  function applyCopy() {
    ...
    if (sheets.length > 0) showReportMockup();
  }
