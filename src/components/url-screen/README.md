...
1. Валидный URL (debounce) → справа белый лист с вертикальной заглушкой «Портфолио» (без скриншотов).
2. Очистка поля → лист улетает вниз.
3. Submit → `onSubmit(url)` (persist), затем анимация как quiz → done: форма уходит, слева «Портфолио отправлено» + «Выйти», лист улетает, зелёный mesh. URL → `/done` через `syncRoute` (без скачка на success-screen).
4. «Выйти» → `onExit` → home.
