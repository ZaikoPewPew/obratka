...
- В меню `displayName` (из `profiles.display_name`) и email не кликабельны; «Настройки» → `/settings`, «Пригласить» → explainer `homeInvite*` (Figma `492:4030`): фото [`currency-referal.jpg`](../../assets/home/modal/currency-referal.jpg) + бар `uses из max` / код + copy / «Поделиться» (share link; без CTA модалки); «Контакты» → `createAppModal`, «Выйти» → полный Supabase `signOut` + очистка локальной сессии.
