...
- CTA «Закинуть своё»: плюс ↔ замок по доступности; слот занят локально → flash+buzz submit без модалки; иначе без монет → buzz submit + чип баланса + flash фона (`motion-control-error-buzz`, `--home-screen-tabbar-submit-bg-error` / `--motion-control-error-buzz-*`); серверный gate — `applyRoute` (`hasFreeMineSlot` параллельно с `reconcileSessionAccess`); notices (no slots / pending limit / already reviewed) — `noticeModal`.
