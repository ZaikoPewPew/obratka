...
| Home: review intro (`homeReviewIntro*`), mine not-ready (`homeMineNotReady*` via notice), invite, submit-locked, reputation explainer, balance explainer (`homeBalance*`), contacts | Complaint-модалка на `report-screen` (свой DOM, не app-modal) |
