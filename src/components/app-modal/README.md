...
| Home: review intro (`homeReviewIntro*`, Figma `492:3611`), mine not-ready (`homeMineNotReady*` via notice), invite explainer (`homeInvite*`, Figma `492:4030`), submit-locked, reputation explainer (`homeReputation*`), balance explainer (`homeBalance*`), p4p online explainer (`homeLegendaryOnline*`), contacts | Complaint-модалка на `report-screen` (свой DOM, не app-modal) |
