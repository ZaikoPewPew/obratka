...
`/review` = просмотр портфолио + таймер **45 s** (`REVIEW_SESSION_SECONDS` в `src/config/review.js`) + опциональная надиктовка (чип rec → `answers.dictation`) + post-edit пунктуации (Edge `polish-dictation`; **клиент сейчас off** — `POLISH_ENABLED = false` в `dictationPolish.js`; soft-fail → сырой текст).  
