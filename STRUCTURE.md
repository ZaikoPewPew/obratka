...
`/home` — feed/mine (рейтинг UI off, `RATING_TAB_ENABLED = false`; `?tab=rating` → feed); активный вид хранится в query через `homeRoute` (`?tab=mine`; `?filter=` игнорируется); SWR-кэш `homeListCache` (`feed`/`mine`/`rating`); aside «Топы в сети»; FAB feedback (`feedback`); кубик «наверх» (`scroll-top`). Intro до claim; `reviewedByMe` исчезает из ленты; mine report gate; на Моих — слот + «Архивные» + completed (`tabs-panel` не монтируется); free-slot до `MAX_MINE_PENDING` (=1); точка на «Чужие посты» (`feedSeen`); точка на «Мои» (`mineReadySeen`); tabbar-dock (glass tabs + «Закинуть своё») с контрастом над превью (`--on-dark`); на open/reload — entrance cascade (`--home-screen-reveal-delay-*`, dock = `motion-reveal-dock` без opacity).
