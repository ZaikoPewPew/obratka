...
| Referrals validate/redeem / share | wired (1 код / 2 слота, seed ops-only, без наград) |
...
| Seed | холодный старт / публичные пачки только через ops SQL в `referral_seed_codes` (живые коды **не** в git) — [`referral-seed-templates.sql`](supabase/sql/referral-seed-templates.sql) |
...
| `public.referral_seed_codes` | bootstrap / публичные пачки ×100; только через RPC; живые коды не в git |
